...
func (s *WebSocketServer) HandleConnect(w http.ResponseWriter, r *http.Request, userID string) {
	ctx := r.Context()
	...
	_ = c.writeJSON(map[string]any{"type": "welcome", "session_id": sessionID})
...
func (s *WebSocketServer) readLoop(ctx context.Context, c *Client) {
	...
	for {
		...
		default:
			_ = c.writeJSON(map[string]any{"type": "echo", "content": msg.Content})
		}
	}
}
...
func (s *WebSocketServer) SendToSession(fromSession, toSession string, content any) error {
	...
	return rcpt.writeJSON(map[string]any{
