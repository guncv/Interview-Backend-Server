...
import (
	...
	utils "gitlab.com/interview-simulation/interview-backend-server/internal/mocks/utils"
	utilsPkg "gitlab.com/interview-simulation/interview-backend-server/internal/utils"
)

func TestResumeService_CreateResumeWithRequirements(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			input: &entities.CreateResumeWithRequirementsRequest{
				...
				Company:         "Google",
				WorkType:        "Full-time",
				JobRequirements: "1 year of experience in software development",
			...
			setup: func() (*utils.MockGenerator, *mockMiddleware.MockAuthContext, *utils.MockValidator, *mockS3.MockS3Storage, *mockResume.MockResumeReposity, *queue.MockRedisTaskPublisher) {
				...
				mockValidator.EXPECT().
					IsAllowedResumeContentType(ctx, mock.AnythingOfType("*multipart.FileHeader")).
					Return(true)
				...
				mockResumeRepository.EXPECT().
					CreateResume(ctx, mock.AnythingOfType("*db.CreateResumeParams")).
					Return(nil)
		...
		{
			name: "Success - Additional resume (not default)",
			input: &entities.CreateResumeWithRequirementsRequest{
				File: &multipart.FileHeader{
				...
				Company:         "Microsoft",
				WorkType:        "Full-time",
				JobRequirements: "5 years of experience",
				InterviewType:   "Technical",
				Language:        "English",
			},
			setup: func() (*utils.MockGenerator, *mockMiddleware.MockAuthContext, *utils.MockValidator, *mockS3.MockS3Storage, *mockResume.MockResumeReposity, *queue.MockRedisTaskPublisher) {
		...
		},
		{
			...
			input: &entities.CreateResumeWithRequirementsRequest{
				...
				Company:         "Google",
				WorkType:        "Full-time",
				JobRequirements: "1 year of experience",
				InterviewType:   "Technical",
				Language:        "English",
			},
			setup: func() (*utils.MockGenerator, *mockMiddleware.MockAuthContext, *utils.MockValidator, *mockS3.MockS3Storage, *mockResume.MockResumeReposity, *queue.MockRedisTaskPublisher) {
		...
		{
			name: "Error - Invalid file content type",
			input: &entities.CreateResumeWithRequirementsRequest{
				File: &multipart.FileHeader{
					Filename: "resume.txt",
					Size:     1024 * 1024,
			...
			verify: func(t *testing.T, gotResp *entities.CreateResumeAndJobRequirementResp, gotErr error) {
				...
				assert.True(t, ok)
				assert.Equal(t, app_error.ErrCodeResumeInvalidFileContentType, appErr.Code)
			},
		},
		{
			...
			verify: func(t *testing.T, gotResp *entities.CreateResumeAndJobRequirementResp, gotErr error) {
				...
				appErr, ok := gotErr.(*app_error.AppError)
				assert.True(t, ok)
				assert.Equal(t, app_error.ErrCodeResumeInvalidFileSize, appErr.Code)
			},
		},
		{
			name: "Error - Check default resume failure",
			input: &entities.CreateResumeWithRequirementsRequest{
				File: &multipart.FileHeader{
					Filename: "resume.pdf",
					Size:     1024 * 1024,
		...
		{
			name: "Error - Create resume failure with cleanup",
			input: &entities.CreateResumeWithRequirementsRequest{
				File: &multipart.FileHeader{
					Filename: "resume.pdf",
					Size:     1024 * 1024,
		...
		{
			name: "Error - Create resume failure with cleanup failure",
			input: &entities.CreateResumeWithRequirementsRequest{
				File: &multipart.FileHeader{
					Filename: "resume.pdf",
					Size:     1024 * 1024,
		...
		{
			name: "Success - File size exactly at limit",
			input: &entities.CreateResumeWithRequirementsRequest{
				File: &multipart.FileHeader{
					Filename: "resume.pdf",
					Size:     int64(constants.ResumeMaxFileSize), // Exactly at limit (5MB)
					Header:   map[string][]string{"Content-Type": {"application/pdf"}},
				},
				Position:        "Software Engineer",
				Company:         "Google",
		...
		{
			name: "Error - File size one byte over limit",
			input: &entities.CreateResumeWithRequirementsRequest{
				File: &multipart.FileHeader{
					Filename: "resume.pdf",
					Size:     int64(constants.ResumeMaxFileSize) + 1, // One byte over limit
					Header:   map[string][]string{"Content-Type": {"application/pdf"}},
				},
				Position:        "Software Engineer",
				Company:         "Google",
		...
		{
			name: "Success - File size one byte under limit",
			input: &entities.CreateResumeWithRequirementsRequest{
				File: &multipart.FileHeader{
					Filename: "resume.pdf",
					Size:     int64(constants.ResumeMaxFileSize) - 1, // One byte under limit
					Header:   map[string][]string{"Content-Type": {"application/pdf"}},
				},
				Position:        "Software Engineer",
				Company:         "Google",
		...
		},
		{
			...
			setup: func() (*utils.MockGenerator, *mockMiddleware.MockAuthContext, *utils.MockValidator, *mockS3.MockS3Storage, *mockResume.MockResumeReposity, *queue.MockRedisTaskPublisher) {
				...
				// No need to mock CheckIsDefaultResumeExistsByUserID since validation fails early
				// No need to mock S3 upload since validation fails early
				// No need to mock UUID generation since validation fails early
				// No need to mock CreateResume since validation fails early
		...
		{
			name: "Error - Negative file size",
			input: &entities.CreateResumeWithRequirementsRequest{
				File: &multipart.FileHeader{
					Filename: "resume.pdf",
					Size:     -1, // Negative file size
					Header:   map[string][]string{"Content-Type": {"application/pdf"}},
				},
				Position:        "Software Engineer",
				Company:         "Google",
	...
	for _, tC := range testCases {
		t.Run(tC.name, func(t *testing.T) {
			mockGenerator, mockAuthContext, mockValidator, mockS3Storage, mockResumeRepository, mockRedisTaskPublisher := tC.setup()
			defer func() {
