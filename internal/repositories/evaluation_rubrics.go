...
type EvaluationRubricsRepository interface {
	GetRubricWithCriteriaByName(ctx context.Context, name string) ([]db.GetRubricWithCriteriaByNameRow, error)
}
...
func (r *evaluationRubricsRepository) GetRubricWithCriteriaByName(ctx context.Context, name string) ([]db.GetRubricWithCriteriaByNameRow, error) {
	r.log.InfoWithID(ctx, "[Repository: GetRubricWithCriteriaByName] Called")

	resp, err := r.db.GetRubricWithCriteriaByName(ctx, name)
	if err != nil {
