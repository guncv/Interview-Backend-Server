...
type SessionRepository interface {
	...
	GetSessionByID(ctx context.Context, id string) (db.Sessions, error)
...
func (r *sessionRepository) GetSessionByID(ctx context.Context, id string) (db.Sessions, error) {
	r.log.InfoWithID(ctx, "[Repository: GetSession] Called")

	session, err := r.db.GetSessionByID(ctx, uuid.MustParse(id))
	if err != nil {
...
func (r *sessionRepository) RevokeSessionByID(ctx context.Context, id string) error {
	r.log.InfoWithID(ctx, "[Repository: RevokeSession] Called")

	if err := r.db.RevokeSessionByID(ctx, uuid.MustParse(id)); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
